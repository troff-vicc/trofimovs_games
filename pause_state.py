...
class PauseState:
    ...
    def draw(self):
        ...
        # Тень под кнопкой
        button_with_shadow = button_img.get_rect(center=(WINDOW_SIZE // 3 + 5, WINDOW_SIZE // 2 + 2 * BORDER_WIDTH + 5))
        pygame.draw.rect(self.screen, GRAY, button_with_shadow, border_radius=TILE_RADIUS)
        # Прямоугольник кнопки (Белый фон)
        self.continue_rect = button_img.get_rect(center=(WINDOW_SIZE // 3, WINDOW_SIZE // 2 + 2 * BORDER_WIDTH))
        pygame.draw.rect(self.screen, BACKGROUND, self.continue_rect, border_radius=TILE_RADIUS)
        self.screen.blit(button_img, self.continue_rect)

        """кнопка ДОМОЙ"""
        button_img = pygame.image.load("resources/house.png")  # PNG с прозрачностью
        # Тень под кнопкой
        button_with_shadow = button_img.get_rect(
            center=(2 * WINDOW_SIZE // 3 + 5, WINDOW_SIZE // 2 + 2 * BORDER_WIDTH + 5))
        pygame.draw.rect(self.screen, GRAY, button_with_shadow, border_radius=TILE_RADIUS)
        # Прямоугольник кнопки (Белый фон)
        self.button_rect = button_img.get_rect(center=(2 * WINDOW_SIZE // 3, WINDOW_SIZE // 2 + 2 * BORDER_WIDTH))
